-- ============================================================
-- CoreBank Phase 1 hardening subset
--
-- Derived from corebank_postgresql_v2.sql as the target-schema
-- direction, but split into a dependency-safe Flyway step.
--
-- Scope in this migration:
-- - partitioning-ready table pattern
-- - saga/orchestration state
-- - CQRS/read-model readiness
-- - field-level encrypted payload storage
-- - limit management
-- - hot-account slotting support
-- - product versioning foundation
-- - reconciliation hardening
--
-- Intentionally deferred from target SQL:
-- - ALTER TABLE loan_contracts ADD COLUMN product_version_id uuid
-- - ALTER TABLE term_deposit_contracts ADD COLUMN product_version_id uuid
--
-- Notes:
-- - V2__base_schema.sql remains the current authoritative financial write-path baseline.
-- - *_p partitioned tables created here define the target partitioning pattern and
--   do not replace V2 base tables yet.
-- ============================================================
...
-- New deployments should use these partitioned tables directly once the
-- application write-path is explicitly moved to them.
...
COMMENT ON TABLE account_balance_snapshots IS
  'Non-authoritative balance snapshots derived from authoritative account and ledger state.';

COMMENT ON TABLE account_read_models IS
  'Non-authoritative read model for teller, dashboard, and search-heavy queries.';
